...
import com.studio.artaban.tagthebus.social.EngSocial;
...
public class EngFacebook extends EngSocial {
	...
	public EngFacebook(PhotoActivity activity, boolean displayError) {
	...
	@Override public boolean login() {
		...
		*/
        ...
        if ((!session.isOpened()) && (!session.isClosed())) {
            ...
            request.setRequestCode(EngSocial.REQUEST_LOGIN);
	...
	private void onRequestCompleted(short request, Response response) {
		...
		switch (request) {
			...
			case REQUEST_SHARE_PICTURE: {
                ...
                //if (mDisplayError)
					Toast.makeText(mActivity, mActivity.getResources().getString(R.string.facebook_shared), Toast.LENGTH_LONG).show();
				//EngLibrary.loadSocial(NETWORK_ID_FACEBOOK, REQUEST_SHARE_VIDEO, REQ_RESULT_SUCCEEDED, (short)0, (short)0,
				//		null);
				break;
			}
	...
	////// Share fields (index)
	static private final short SHARE_FIELD_NAME = 0; // Link name (REQUEST_SHARE_LINK) & Video/Picture file name (REQUEST_SHARE_VIDEO/REQUEST_SHARE_PICTURE)
	...
	public boolean sharePicture(String[] data) {
        ...
        if (data == null) {
    		...
    		Log.w("EngFacebook", "No picture data to share");
            if (mDisplayError)
        ...
        Bitmap picToShare = BitmapFactory.decodeFile(data[SHARE_FIELD_NAME]);
        ...
        reqShare.getParameters().putString("message", data[SHARE_FIELD_CAPTION] + " (" + data[SHARE_FIELD_DESCRIPTION] + ")");
        reqShare.executeAsync();
		return true;
	}
	@Override public boolean shareVideo(String[] data) {
        ...
        Request reqShare;
		try {
			reqShare = Request.newUploadVideoRequest(Session.getActiveSession(), videoFile,
					new Request.Callback() {
			        public void onCompleted(Response response) {

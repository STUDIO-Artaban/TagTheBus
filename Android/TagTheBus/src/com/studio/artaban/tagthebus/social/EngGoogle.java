package com.studio.artaban.tagthebus.social;
...
import com.studio.artaban.tagthebus.PhotoActivity;
...
public class EngGoogle extends EngSocial implements GoogleApiClient.ConnectionCallbacks,
	...
	public EngGoogle(PhotoActivity activity, boolean displayError) {
	...
	*/
	...
	static public final int SIGN_IN_REQUEST = 0;
	...
	public void launch(int req, int res, Intent data) {
		switch (req) {
			...
			case SHARE_PICTURE_REQUEST: {

	        	if (res != Activity.RESULT_OK) {
	                //if (mDisplayError)
						Toast.makeText(mActivity, mActivity.getResources().getString(R.string.error_shared), Toast.LENGTH_LONG).show();
					//EngLibrary.loadSocial(NETWORK_ID_GOOGLE, REQUEST_SHARE_VIDEO, REQ_RESULT_FAILED, (short)0, (short)0, null);
	        	}
	        	else {
	                //if (mDisplayError)
						Toast.makeText(mActivity, mActivity.getResources().getString(R.string.google_shared), Toast.LENGTH_LONG).show();
					//EngLibrary.loadSocial(NETWORK_ID_GOOGLE, REQUEST_SHARE_VIDEO, REQ_RESULT_SUCCEEDED, (short)0, (short)0, null);
	        	}
				break;
			}
	...
	////// Share
	static private final short SHARE_FIELD_URL = 0; // Link URL (REQUEST_SHARE_LINK) & Video file name (REQUEST_SHARE_VIDEO)
	static private final short SHARE_FIELD_TYPE = 1; // Video mime type (REQUEST_SHARE_VIDEO)
	static private final short SHARE_FIELD_TITLE = 2; // Video title (REQUEST_SHARE_VIDEO)
	...
	public boolean sharePicture(String[] data) {
        ...
        Uri pictureURI = mActivity.getURI(data[SHARE_FIELD_URL], data[SHARE_FIELD_TYPE], data[SHARE_FIELD_TITLE]);
        if (pictureURI == null) {
		...
		Intent shareIntent = new PlusShare.Builder(mActivity)
			.setText(data[SHARE_FIELD_TYPE] + " (" + data[SHARE_FIELD_TITLE] + ")")
	...
	@Override public boolean shareVideo(String[] data) {
        ...
        /*
        Uri videoURI = mActivity.saveMOV(data[SHARE_FIELD_URL], data[SHARE_FIELD_TYPE], data[SHARE_FIELD_TITLE]);
        if (videoURI == null) {
